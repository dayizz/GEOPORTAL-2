...
import '../../predios/data/predios_firestore_repository.dart';
...
import '../../propietarios/data/propietarios_firestore_repository.dart';
import '../../propietarios/providers/propietarios_provider.dart';
import '../../carga/utils/geojson_mapper.dart';
import '../providers/mapa_provider.dart';

Map<String, dynamic> normalizeFeatureProperties(
...
class _MapaScreenState extends ConsumerState<MapaScreen> {
  ...
  static final Expando<_ImportedGeometryCacheEntry> _importedGeometryCache =
      Expando<_ImportedGeometryCacheEntry>('importedGeometryCache');
  ...
  bool _showPredioClaveLabels = true;
  ...
  final TextEditingController _kmInicioCtrl = TextEditingController(
    text: '0+000',
  );
  final TextEditingController _kmFinCtrl = TextEditingController(text: '0+000');
  final TextEditingController _rotationDegreesCtrl = TextEditingController(
    text: '0',
  );
  final FocusNode _rotationDegreesFocus = FocusNode();
  String? _proyecto;
  ...
  static const _defaultRotation = 0.0;

  bool _initialFitDone = false;
  double _currentZoom = _defaultZoom;
  double _currentRotation = _defaultRotation;
  ...
  List<_PredioVisualData>? _lastPredioLabelVisuals;
  List<Marker>? _lastPredioLabelMarkers;
  ...
  void dispose() {
    ...
    _kmFinCtrl.dispose();
    _rotationDegreesCtrl.dispose();
    _rotationDegreesFocus.dispose();
  ...
  Widget build(BuildContext context) {
    ...
    _focusImportedIfNeeded(importedData.bounds, importedFeatures);
      ...
      prediosAsync.whenData((predios) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          ...
          setState(() {
            ...
            _manualPredioSearchCtrl.text = target != null
                ? _manualPredioLabel(target)
                : '';
          });
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Selecciona un poligono huérfano en el mapa y luego pulsa Vincular.',
              ),
            ),
          );
        });
    ...
    return AppScaffold(
      ...
      child: Stack(
        children: [
          prediosAsync.when(
            data: (predios) {
              _schedulePredioGeometryWarmup(predios);
              ...
              return FlutterMap(
                ...
                options: MapOptions(
                  ...
                  initialRotation: _defaultRotation,
                  onPositionChanged: (position, hasGesture) {
                    ...
                    final nextRotation = _normalizeRotationDegrees(
                      position.rotation,
                    );
                    if ((nextRotation - _currentRotation).abs() >= 0.1) {
                      _currentRotation = nextRotation;
                      if (!_rotationDegreesFocus.hasFocus) {
                        _rotationDegreesCtrl.text =
                            _currentRotation.round().toString();
                      }
                    }

                    final importedCount = ref
                        .read(importedFeaturesProvider)
                        .length;
                    if (importedCount > 0) {
                      _scheduleViewportStatusSync(
                  ...
                  onTap: (_, point) {
                    ...
                    setState(() {
                      if (_isManualLinkMode) {
                        final currentImported = ref.read(
                          importedFeaturesProvider,
                        );
                        final importedIdx = _findImportedAtPoint(
                          point,
                          currentImported,
                        );
                        if (importedIdx != null) {
                          ...
                          if (_isImportedFeatureLinked(feature)) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text(
                                  'Ese poligono ya esta vinculado a un registro de Gestion.',
                                ),
                              ),
                            );
                            return;
                      ...
                      if (tappedVisual != null) {
                        ...
                        if (_isDrawing && tappedVisual.rings.isNotEmpty) {
                          final selectedPoints = List<LatLng>.from(
                            tappedVisual.rings.first,
                          );
                          ...
                          _isDrawing = false;
                          _detectedAreaM2 = _calculateAreaSquareMeters(
                            _draftPoints,
                          );
                          ...
                          _tipoPropiedad =
                              ...
                              : null;
                          _proyecto =
                              _normalizeProyecto(predio.proyecto) ??
                ...
                children: [
                  ...
                  PolygonLayer(
                    polygons: visuals.expand((v) => v.polygons).toList(),
                  ),
                  ...
                  if (_showPredioClaveLabels)
                    MarkerLayer(markers: _buildPredioClaveMarkers(visuals)),
                  if (_showPredioClaveLabels)
                    MarkerLayer(
                      markers: _buildImportedClaveMarkers(importedFeatures),
                    ),
                  ...
                  if (_draftPoints.length >= 3)
                    PolygonLayer(
                      polygons: [
                        Polygon(
                          points: _draftPoints,
                          color: _draftPolygonColor(
                            colorMode,
                          ).withValues(alpha: 0.58),
                          borderColor: _draftPolygonColor(
                            colorMode,
                  ...
                  children: [
                    ...
                    Material(
                      ...
                      child: InkWell(
                        borderRadius: BorderRadius.circular(10),
                        onTap: () => setState(() {
                          _showPredioClaveLabels = !_showPredioClaveLabels;
                        }),
                        child: Container(
                          width: 80,
                          height: 40,
          ...
          Positioned(
            bottom: _selectedPredio != null ? 174 : 24,
            right: 24,
            child: _buildCompassRose(_currentRotation),
          ),
          Positioned(
            bottom: _selectedPredio != null ? 210 : 60,
            right: 24,
            child: _buildMapRotationInput(),
          ),
          Positioned(top: 16, left: 16, child: _buildCapturaToggleButton()),
          if (_showCapturaModal)
            Positioned(top: 72, left: 16, child: _buildCapturaModal()),
          ...
          if (ref.watch(importacionEstadoProvider) ==
              ImportacionEstado.procesando)
            Positioned.fill(
              child: ColoredBox(
        ...
        children: [
          Transform.rotate(
            ...
            child: Stack(
              alignment: Alignment.center,
              children: [
                CustomPaint(size: const Size(86, 86), painter: _CompassStarPainter()),
                _buildDirectionLabel('N', Alignment.topCenter, highlighted: true),
                _buildDirectionLabel('S', Alignment.bottomCenter),
                _buildDirectionLabel('E', Alignment.centerRight),
                _buildDirectionLabel('O', Alignment.centerLeft),
              ],
            ),
          ),
        ],
    ...
    return Material(
      ...
      child: Container(
        ...
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Icon(Icons.rotate_right, size: 18, color: Color(0xFF555555)),
            const SizedBox(width: 6),
            SizedBox(
              width: 72,
              child: TextField(
                controller: _rotationDegreesCtrl,
                focusNode: _rotationDegreesFocus,
                ...
                onSubmitted: (_) => _applyRotationFromInput(),
                onTapOutside: (_) => _applyRotationFromInput(),
              ),
            ),
          ],
        ),
  ...
  void _applyRotationFromInput() {
    final raw = _rotationDegreesCtrl.text.trim().replaceAll('°', '');
    final parsed = double.tryParse(raw);
    if (parsed == null) {
      _rotationDegreesCtrl.text = _currentRotation.round().toString();
      return;
    }

    final target = _normalizeRotationDegrees(parsed);
    _currentRotation = target;
    _rotationDegreesCtrl.text = target.round().toString();
    _mapCtrl.rotate(target);
  }

  double _normalizeRotationDegrees(double? value) {
    final v = (value ?? 0) % 360;
    return v < 0 ? v + 360 : v;
  }
  ...
  _VisualLayerData _buildVisualData(List<Predio> predios, MapaColorMode mode) {
    ...
    for (final predio in predios) {
      ...
      final polygonRingsList = _simplifyPolygonsForZoom(
        geometryCache.polygons,
        zoomBucket,
      );
      final primaryRings =
          _largestPolygonRings(polygonRingsList) ?? const <List<LatLng>>[];
      final polygons = <Polygon>[];
      for (final rings in polygonRingsList) {
      ...
      visuals.add(
        _PredioVisualData(
          ...
          color: color,
          rings: primaryRings,
          polygonRingsList: polygonRingsList,
          markerPoint: geometryCache.markerPoint,
          labelPoints: geometryCache.labelPoints,
          polygons: polygons,
        ),
      );
    }
  ...
  _ImportedLayerData _buildImportedLayerData({
    ...
    if (kDebugMode && features.isNotEmpty) {
      ...
      final geomType = geom?.runtimeType?.toString() ?? 'null';
      final geomKeys = geom is Map ? (geom as Map).keys.take(5).toList() : [];
      final geomGeoType = geom is Map ? geom['type'] : 'N/A';
      final coords = geom is Map ? geom['coordinates'] : null;
      final coordType = coords?.runtimeType?.toString() ?? 'null';
      final coordLen = coords is List ? (coords as List).length : -1;
      debugPrint(
        '[geom_debug] feature[0]: geom=$geomType keys=$geomKeys geoType=$geomGeoType coords=$coordType len=$coordLen',
      );
      if (coords is List && (coords as List).isNotEmpty) {
        final ring0 = (coords as List).first;
        final ring0Type = ring0?.runtimeType?.toString() ?? 'null';
        final ring0Len = ring0 is List ? (ring0 as List).length : -1;
        debugPrint('[geom_debug] ring[0]: type=$ring0Type len=$ring0Len');
        if (ring0 is List && (ring0 as List).isNotEmpty) {
          final pt0 = (ring0 as List).first;
          debugPrint(
            '[geom_debug] point[0]: runtimeType=${pt0.runtimeType} val=$pt0',
          );
        }
      }
    }
    ...
    for (int i = 0; i < features.length; i++) {
      ...
      final extractedPolygons = _simplifyPolygonsForZoom(
        geometryCache.polygons,
        zoomBucket,
      );
      ...
      for (final rings in extractedPolygons) {
        if (rings.isEmpty || rings.first.length < 3) continue;
        final signature = _polygonRingsSignature(rings);
        if (!renderedPolygonSignatures.add(signature)) {
          continue;
        }
        vertices += _countRingVertices(rings);
        polygons.add(
  ...
  _PredioVisualData? _findVisualAtPoint(
    LatLng point,
    List<_PredioVisualData> visuals,
  ) {
    for (final visual in visuals.reversed) {
      for (final rings in visual.polygonRingsList.reversed) {
        if (rings.isEmpty) continue;
        final outerRing = rings.first;
        if (!_pointInRing(point, outerRing)) continue;

        final insideHole = rings
            .skip(1)
            .any((ring) => _pointInRing(point, ring));
        if (!insideHole) {
          return _PredioVisualData(
            predio: visual.predio,
            color: visual.color,
            rings: rings,
            polygonRingsList: visual.polygonRingsList,
            markerPoint: _pointForPolygonRings(rings) ?? visual.markerPoint,
            labelPoints: visual.labelPoints,
            polygons: visual.polygons,
          );
        }
      }
    }
  ...
  List<Marker> _buildSelectedPredioMarkers(_PredioVisualData? selectedVisual) {
    ...
    final markerPoint = selectedVisual.markerPoint;
  ...
  LatLng? _pointForPolygonRings(List<List<LatLng>> rings) {
    ...
    if (outerCentroid != null &&
        _isPointInPolygonWithHoles(outerCentroid, rings)) {
    ...
    final lat =
        outer.map((p) => p.latitude).reduce((a, b) => a + b) / outer.length;
    final lng =
        outer.map((p) => p.longitude).reduce((a, b) => a + b) / outer.length;
    ...
    for (double x = minLng; x < maxLng; x += cellSize) {
      for (double y = minLat; y < maxLat; y += cellSize) {
        final c = _PolylabelCell(
          ...
          y + cellSize / 2,
          cellSize / 2,
          _signedDistanceToPolygonEdges(
            LatLng(y + cellSize / 2, x + cellSize / 2),
            rings,
          ),
        );
        cells.add(c);
    ...
    while (cells.isNotEmpty) {
      ...
      cells.addAll([
        _PolylabelCell(
          ...
          cell.y - h,
          h,
          _signedDistanceToPolygonEdges(LatLng(cell.y - h, cell.x - h), rings),
        ),
        _PolylabelCell(
          cell.x + h,
          cell.y - h,
  ...
  void _flyToPredio(Predio predio) {
    try {
      final polygons = _getPredioGeometryCache(predio).polygons;
  ...
  void _focusImportedIfNeeded(
    ...
    final identity = identityHashCode(importedFeatures);
  ...
  _PredioGeometryCacheEntry _getPredioGeometryCache(Predio predio) {
    ...
    final polygons = _extractPolygons(predio.geometry);
    final entry = _PredioGeometryCacheEntry(
      polygons: polygons,
      markerPoint: _markerPoint(predio, polygons),
      labelPoints: _labelPointsFromPolygons(polygons),
    );
  ...
  void _schedulePredioGeometryWarmup(List<Predio> predios) {
    ...
    compute(_preparePredioGeometryWarmup, payload)
        .then((entries) {
          if (!mounted) return;
          for (final entry in entries) {
            final key = entry['key']?.toString();
            final polygonsRaw = entry['polygons'];
            if (key == null || polygonsRaw is! List) continue;
            if (_predioGeometryCache.containsKey(key)) continue;

            final polygons = <List<List<LatLng>>>[];
            for (final polygonRaw in polygonsRaw.whereType<List>()) {
              final rings = <List<LatLng>>[];
              for (final ringRaw in polygonRaw.whereType<List>()) {
                final ring = <LatLng>[];
            ...
            _predioGeometryCache[key] = _PredioGeometryCacheEntry(
              polygons: polygons,
              markerPoint: _markerPointFromPolygons(polygons),
              labelPoints: _labelPointsFromPolygons(polygons),
            );
          }
          if (kDebugMode) {
  ...
  _ImportedGeometryCacheEntry _getImportedGeometryCache(
    ...
    final cached = _importedGeometryCache[feature];
    if (cached != null) return cached;

    final entry = _ImportedGeometryCacheEntry(
      polygons: _extractImportedPolygons(feature),
    );
    _importedGeometryCache[feature] = entry;
    return entry;
  }
  ...
  List<List<List<LatLng>>> _extractImportedPolygons(
    Map<String, dynamic> feature,
  ) {
    final geometries = <Map<String, dynamic>>[];

    final directGeometry = _geometryAsMap(feature['geometry']);
    if (directGeometry != null) geometries.add(directGeometry);

    final altGeometry = _geometryAsMap(
      feature['_geometry'] ?? feature['geom'] ?? feature['geometria'],
    );
    if (altGeometry != null) geometries.add(altGeometry);

    final props = feature['properties'];
    if (props is Map) {
      final propsMap = Map<String, dynamic>.from(props);
      final nestedGeometry = _geometryAsMap(
        propsMap['geometry'] ??
            propsMap['_geometry'] ??
            propsMap['geom'] ??
            propsMap['geometria'],
      );
      if (nestedGeometry != null) geometries.add(nestedGeometry);
    }

    final result = <List<List<LatLng>>>[];
    for (final geometry in geometries) {
      final extracted = _extractPolygons(geometry);
    ...
    // Soporte adicional para GeoJSON anidados como Feature / FeatureCollection / GeometryCollection.
    final featureType = feature['type']?.toString();
    if (featureType == 'FeatureCollection') {
      final childFeatures = feature['features'];
      if (childFeatures is List) {
        for (final child in childFeatures.whereType<Map>()) {
          final childMap = Map<String, dynamic>.from(child);
          result.addAll(_extractImportedPolygons(childMap));
        }
      }
      return result;
    }
    ...
    if (featureType == 'Feature') {
      final innerGeometry = _geometryAsMap(feature['geometry']);
      if (innerGeometry != null) {
        return _extractPolygons(innerGeometry);
      }
    }

    final geometryType = directGeometry?['type']?.toString();
    if (geometryType == 'GeometryCollection') {
      final geometriesRaw = directGeometry?['geometries'];
      if (geometriesRaw is List) {
        for (final child in geometriesRaw.whereType<Map>()) {
          final childGeometry = Map<String, dynamic>.from(child);
          result.addAll(_extractPolygons(childGeometry));
        }
      }
    }
  ...
  List<LatLng> _ringToLatLng(List<dynamic> ring) {
    try {
      ...
      for (final coord in ring) {
        ...
        if (coord is Map) {
          final map = Map<String, dynamic>.from(coord);
          final x = _parseCoord(
            map['lng'] ??
                map['lon'] ??
                map['long'] ??
          ...
          final y = _parseCoord(map['lat'] ?? map['y'] ?? map['latitude']);
          if (x == null || y == null || x.isNaN || y.isNaN) continue;
          pairs.add((x, y));
        }
      }
  ...
  LatLng? _markerPoint(Predio predio, List<List<List<LatLng>>> polygons) {
    final polygonPoint = _markerPointFromPolygons(polygons);
  ...
  LatLng? _markerPointFromPolygons(List<List<List<LatLng>>> polygons) {
    final largestRings = _largestPolygonRings(polygons);
    if (largestRings == null || largestRings.isEmpty) return null;
    return _pointForPolygonRings(largestRings);
  }

  List<LatLng> _labelPointsFromPolygons(List<List<List<LatLng>>> polygons) {
    final points = <LatLng>[];
    for (final rings in polygons) {
      if (rings.isEmpty) continue;
      final point = _pointForPolygonRings(rings);
      if (point != null) {
        points.add(point);
      }
    }
  ...
  List<List<LatLng>>? _largestPolygonRings(List<List<List<LatLng>>> polygons) {
    List<List<LatLng>>? best;
    var bestArea = -1.0;
    for (final rings in polygons) {
      if (rings.isEmpty || rings.first.length < 3) continue;
      var area = _ringSignedArea(rings.first).abs();
  ...
  Widget _buildMarkerDot(Color color) {
    return Icon(
      ...
      size: 34,
      color: color,
      shadows: [Shadow(color: color.withValues(alpha: 0.38), blurRadius: 8)],
    );
  }
          ...
          child: Column(
            ...
            children: [
              ...
              _layerButton(
                ...
                onTap: () {
                  ref.read(mapaBaseLayerProvider.notifier).state =
                      MapaBaseLayer.estandar;
              ...
              const SizedBox(height: 8),
              _layerButton(
                title: 'Clave de predio',
                subtitle: _showPredioClaveLabels ? 'Visible' : 'Oculta',
                icon: Icons.sell_outlined,
                selected: _showPredioClaveLabels,
                onTap: () {
                  setState(() {
                    _showPredioClaveLabels = !_showPredioClaveLabels;
                  });
  ...
  List<Marker> _buildPredioClaveMarkers(List<_PredioVisualData> visuals) {
    if (_lastPredioLabelVisuals == visuals && _lastPredioLabelMarkers != null) {
      return _lastPredioLabelMarkers!;
    }

    final markers = <Marker>[];
    for (final visual in visuals) {
      final rawClave = visual.predio.claveCatastral.trim();
      final clave = rawClave.isEmpty ? 'N/D' : rawClave;

      final labelPoints = visual.labelPoints.isNotEmpty
          ? visual.labelPoints
          : (visual.markerPoint != null
                ? [visual.markerPoint!]
                : const <LatLng>[]);
    ...
    _lastPredioLabelVisuals = visuals;
    _lastPredioLabelMarkers = markers;
    return markers;
  }

  List<Marker> _buildImportedClaveMarkers(List<Map<String, dynamic>> features) {
    final markers = <Marker>[];
    ...
    for (final feature in features) {
      ...
      final centroid = _centroidOfPolygons(polygons);
      if (centroid == null) continue;

      final signature = polygons
          .map((rings) => _polygonRingsSignature(rings))
          .join('||');
      if (!renderedSignatures.add(signature)) {
        continue;
      }

      final clave = _extractImportedClaveLabel(feature);
      markers.add(
        Marker(
          point: centroid,
          width: 170,
  ...
  String _extractImportedClaveLabel(Map<String, dynamic> feature) {
    final rawProps = feature['properties'];
    final propsMap = rawProps is Map
        ? Map<String, dynamic>.from(rawProps)
        : <String, dynamic>{};
    final allProps = _flattenFeatureProps(feature, propsMap);

    final clave = _propValue(allProps, const [
      'clave_catastral_db',
      '_clave_catastral',
      '_clavecatastral',
      'clave_catastral',
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    predio.claveCatastral.trim().isEmpty
                        ? 'N/D'
                        : predio.claveCatastral,
                  ),
                ),
            ...
            // 2. Proyecto
            if (predio.proyecto != null &&
                predio.proyecto!.trim().isNotEmpty) ...[
              const SizedBox(height: 2),
              Row(
                children: [
                  const Text(
                  ...
                  Expanded(child: Text(predio.proyecto!)),
                ],
              ),
            ],
            // 3. Propietario
            if (predio.nombrePropietario.trim().isNotEmpty) ...[
              const SizedBox(height: 2),
              Row(
                children: [
                  const Text(
                  ...
                  Expanded(child: Text(predio.nombrePropietario)),
                ],
              ),
            ],
            // 4. Tramo, Segmento o Frente
            if (predio.tramo.trim().isNotEmpty) ...[
              const SizedBox(height: 2),
              Row(
                children: [
                  const Text(
                  ...
                  Expanded(child: Text(predio.tramo)),
                ],
              ),
            ],
            // 5. km inicio, km fin, km lineal
            const SizedBox(height: 2),
            Row(
              children: [
                const Text(
                ...
                Text(predio.kmInicio?.toStringAsFixed(3) ?? 'N/D'),
                const SizedBox(width: 8),
                const Text(
                  'Km fin: ',
                  style: TextStyle(fontWeight: FontWeight.bold),
                ),
                Text(predio.kmFin?.toStringAsFixed(3) ?? 'N/D'),
                const SizedBox(width: 8),
                const Text(
                  'Km lineal: ',
                  style: TextStyle(fontWeight: FontWeight.bold),
                ),
                Text(predio.kmLineales?.toStringAsFixed(3) ?? 'N/D'),
              ],
            ),
            // 6. m2
            const SizedBox(height: 2),
              ...
              children: [
                ...
                Text(
                  predio.superficie != null
                      ? '${predio.superficie!.toStringAsFixed(2)} m²'
                      : 'N/D',
                ),
              ],
            ),
            ...
            Row(
              children: [
                ...
                Text(predio.estatusGestion),
              ],
            ),
            // 8. Identificación, levantamiento, negociación
            const SizedBox(height: 8),
            Wrap(
              spacing: 8,
              runSpacing: 6,
              children: [
                _statusChip('Identificación', predio.identificacion),
                _statusChip('Levantamiento', predio.levantamiento),
                _statusChip('Negociación', predio.negociacion),
              ],
            ),
  ...
  Widget _buildCapturaModal() {
    final area = _detectedAreaM2 > 0
        ? _detectedAreaM2
        : _calculateAreaSquareMeters(_draftPoints);
    final predios =
        ref.watch(prediosMapaProvider).asData?.value ?? const <Predio>[];
          ...
          children: [
            ...
            SizedBox(
              ...
              child: OutlinedButton.icon(
                ...
                icon: Icon(
                  _isDrawing ? Icons.close : Icons.gesture_outlined,
                  size: 16,
                ),
                label: Text(
                  _isDrawing ? 'Cancelar seleccion' : 'Seleccionar poligono',
                ),
                  ...
                  children: [
                    ...
                    Autocomplete<Predio>(
                      ...
                      onSelected: (selected) {
                        setState(() {
                          ...
                          _manualPredioSearchCtrl.text = _manualPredioLabel(
                            selected,
                          );
                        });
                      },
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        ...
                        label: Text(
                          _isLinkingManual ? 'Vinculando...' : 'Vincular',
                        ),
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 8),
                Expanded(
                  child: _buildKmField(label: 'KM fin', controller: _kmFinCtrl),
                ),
              ],
            ),
            const SizedBox(height: 6),
  ...
  bool _sameGeometryMap(Map<String, dynamic>? a, Map<String, dynamic>? b) {
  ...
  List<Map<String, dynamic>> _removeImportedDuplicatesAfterLink({
    ...
    for (var i = 0; i < imported.length; i++) {
      ...
      final samePoligono =
          _poligonoIdFromFeature(importedFeatures: imported, index: i) ==
          linkedPoligonoId;
  ...
  Future<void> _vincularPoligonoManual(List<Predio> predios) async {
    ...
    if (geometry == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('El poligono seleccionado no tiene geometria valida.'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      if (predio.id.startsWith('local-')) {
        ...
        await ref
            .read(prediosFirestoreRepositoryProvider)
            .vincularPoligonoConPredio(predio.id, idPoligono);
      }

      final removedLocalDuplicates = ref
          .read(localPrediosProvider.notifier)
          .removeDuplicatesAfterManualLink(
  ...
  int _similarityScore(String candidateKey, String expectedKey) {
    ...
    if (candidateNorm.contains(expectedNorm) ||
        expectedNorm.contains(candidateNorm)) {
      return 88;
    }
  ...
  Future<void> _saveSelectedPolygon() async {
    ...
    if (_selectedPredio == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Selecciona un predio antes de guardar el poligono.'),
        ),
      );
      return;
    }
    ...
    final oficioConProyecto = _mergeOficioProyectoTag(
      selected.oficio,
      proyectoNormalizado,
    );
    ...
    try {
      ...
      final superficieDetectada = _detectedAreaM2 > 0
          ? _detectedAreaM2
          : _calculateAreaSquareMeters(_draftPoints);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Poligono guardado correctamente en el predio seleccionado.',
          ),
        ),
      );
    } catch (e) {
  ...
  String _polygonRingsSignature(List<List<LatLng>> rings) {
    return rings.map(_ringSignature).join('|');
  }
  ...
  String _ringSignature(List<LatLng> ring) {
    ...
    for (var i = 1; i < normalized.length; i++) {
      if (normalized[i].compareTo(normalized[minIdx]) < 0) {
        minIdx = i;
      }
    }

    final rotated = <String>[
      ...normalized.sublist(minIdx),
      ...normalized.sublist(0, minIdx),
    ];
    final reversed = rotated.reversed.toList(growable: false);
    final forwardKey = rotated.join(';');
    final reverseKey = reversed.join(';');
  ...
  void _openCapturaForImportedFeature(Map<String, dynamic> feature, int idx) {
    ...
    final points = rings.isNotEmpty
        ? List<LatLng>.from(rings.first)
        : <LatLng>[];
    if (points.length > 1 &&
    ...
    setState(() {
      ...
      _estadoCtrl.text =
          _propValue(allProps, ['estado', 'entidad', 'state']) ?? '';
      _municipioCtrl.text =
          _propValue(allProps, [
            'municipio',
            'municipality',
          ...
          '';
      _tipoPropiedad = _propValue(allProps, [
        'tipo_propiedad',
        'tipopropiedad',
        'uso_suelo',
        'usosuelo',
      ...
      _proyecto =
          _normalizeProyecto(proyectoDetectado) ??
          _inferProyectoFromText(
            [
              proyectoDetectado ?? '',
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text(
          'Este poligono no tiene registro en Gestion. Completa la captura para vincularlo.',
        ),
      ),
    );
  }
  ...
  Future<void> _saveImportedFeatureAsPredio() async {
    ...
    try {
      final isDemo = ref.read(demoModeProvider);
      if (!isDemo) {
        final propietariosRepo = ref.read(
          propietariosFirestoreRepositoryProvider,
        );
        final propietario = await propietariosRepo.findOrCreateByNombreCompleto(
          propietarioNombre,
        );
        ...
        await repo.createPredio({
          ...
          'tramo': _tramoCtrl.text.trim().isEmpty
              ? 'T1'
              : _tramoCtrl.text.trim(),
  ...
  Map<String, dynamic> _polygonToGeoJson(List<LatLng> points) {
    final closed = points.first == points.last
        ? points
        : [...points, points.first];
    final ring = closed.map((p) => [p.longitude, p.latitude]).toList();
    return {
      'type': 'Polygon',
      'coordinates': [ring],
    };
  ...
  Future<void> _autofillEstadoMunicipioDesdePoligono() async {
    ...
    try {
      ...
      final estado = (address['state'] ?? address['region'] ?? '')
          .toString()
          .trim();
      final municipio =
          (address['city'] ??
  ...
  LatLng? _polygonCentroid(List<LatLng> points) {
    if (points.length < 3) return null;
    final clean = points.first == points.last
        ? points.sublist(0, points.length - 1)
        : points;
    if (clean.isEmpty) return null;

    final lat =
        clean.map((p) => p.latitude).reduce((a, b) => a + b) / clean.length;
    final lng =
        clean.map((p) => p.longitude).reduce((a, b) => a + b) / clean.length;
    return LatLng(lat, lng);
  }
  ...
  Color _importedFeatureColor(
    ...
    final rawStatusSource =
        allProps['_estatusColorKey']?.toString() ??
        _rawStatusFromGeoJson(allProps);
    ...
    final noLiberado =
        _featureBoolValue(allProps, ['negociacion', '_negociacion']) == true ||
        _featureBoolValue(allProps, ['levantamiento', '_levantamiento']) ==
            true ||
        _featureBoolValue(allProps, ['identificacion', '_identificacion']) ==
            true;
    if (noLiberado) {
  ...
  String? _rawStatusFromGeoJson(Map<String, dynamic> props) {
    ...
    for (final entry in props.entries) {
      ...
      final looksLikeStatusKey =
          normalizedKey == 'estatus' ||
  ...
  String? _normalizeEstatusText(String? value) {
    ...
    if (compact.contains('no aplica') ||
        compact.contains('sin dato') ||
        compact.contains('n/a')) {
      return 'Sin estatus';
    }
    ...
    _apiClient
        ...
        .then((items) {
          ...
          for (final item in items) {
            ...
            final clave = (item['clave_catastral'] ?? '')
                .toString()
                .trim()
                .toUpperCase();
  ...
  void _syncImportedStatusesBatch(List<Map<String, dynamic>> features) {
    ...
    if (_lastImportedBatchSyncIdentity == identity ||
        _isSyncingImportedStatuses) {
      return;
    }
  ...
  (String?, String?) _extractFeaturePredioRef(Map<String, dynamic> allProps) {
    final predioId = (allProps['_predioId'] ?? allProps['predio_id'])
        ?.toString()
        .trim();
    final clave =
        (allProps['clave_catastral_db'] ??
            ...
            .toUpperCase();
    return (
      predioId?.isEmpty == true ? null : predioId,
...
class _SavedPolygon {
  ...
  const _SavedPolygon({required this.points, this.estatus, this.tipoPropiedad});
}

class _PredioGeometryCacheEntry {
  final List<List<List<LatLng>>> polygons;
  final LatLng? markerPoint;
  final List<LatLng> labelPoints;
...
class _PredioVisualData {
  ...
  final List<List<List<LatLng>>> polygonRingsList;
  final LatLng? markerPoint;
  final List<LatLng> labelPoints;
  ...
  const _PredioVisualData({
    ...
    required this.color,
    required this.rings,
    required this.polygonRingsList,
    required this.markerPoint,
    required this.labelPoints,
    required this.polygons,
  });
...
class _VisualLayerData {
  ...
  const _VisualLayerData({required this.visuals, required this.bounds});
}
...
List<Map<String, dynamic>> _preparePredioGeometryWarmup(
  ...
  for (final row in payload) {
    ...
    final polygons = _warmupExtractPolygons(geometry);
    output.add(<String, dynamic>{'key': key, 'polygons': polygons});
  }
  return output;
}

List<List<List<List<double>>>> _warmupExtractPolygons(dynamic geometryRaw) {
  ...
  final targetPolygons = <List<List<dynamic>>>[];
  if (type == 'Polygon') {
    targetPolygons.add(
      coords.whereType<List<dynamic>>().toList(growable: false),
    );
  } else if (type == 'MultiPolygon') {
    for (final polygonRaw in coords.whereType<List>()) {
      targetPolygons.add(
        polygonRaw.whereType<List<dynamic>>().toList(growable: false),
      );
    }
  }

  if (targetPolygons.isEmpty) return const [];

  final polygons = <List<List<List<double>>>>[];
  for (final targetRings in targetPolygons) {
...
class _CompassStarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final outerPrimary = size.width * 0.49;
    final outerDiagonal = size.width * 0.34;
    final innerPrimary = size.width * 0.16;
    final innerDiagonal = size.width * 0.19;

    Offset _point(double angle, double radius) => Offset(
    ...
    for (var i = 0; i < 8; i++) {
      ...
      final tipRadius = isCardinal ? outerPrimary : outerDiagonal;
      final baseRadius = isCardinal ? innerPrimary : innerDiagonal;
      final spread = isCardinal ? (math.pi / 14) : (math.pi / 13);

      final p1 = _point(angle - spread, baseRadius);
      final p2 = _point(angle, tipRadius);
      final p3 = _point(angle + spread, baseRadius);
      ...
      final fill = Paint()
        ...
        ..color = isCardinal
            ? (i % 4 == 0 ? const Color(0xFF1F1F1F) : const Color(0xFFF5F5F5))
            : const Color(0xFFD9D9D9);
      final stroke = Paint()
    ...
    canvas.drawCircle(center, 3.8, centerPaint);
  }
