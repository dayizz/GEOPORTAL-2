...
class _CargaArchivoScreenState extends ConsumerState<CargaArchivoScreen> {
  ...
  final Set<String> _archivosSeleccionados = <String>{};
  ...
  void _toggleArchivoSeleccionado(String fileId, bool selected) {
    if (_loading || _sincronizando || _eliminando) return;
    setState(() {
      if (selected) {
        _archivosSeleccionados.add(fileId);
      } else {
        _archivosSeleccionados.remove(fileId);
      }
    });
  }

  void _limpiarSeleccionArchivos() {
  ...
  Future<void> _seleccionarArchivo() async {
    ...
    try {
      ...
      if (ext == 'xlsx' || ext == 'xls') {
        ...
        debugPrint(
          '[GEOJSON] archivo seleccionado: ${file.name} (ext=$ext, bytes=${bytes.length})',
        );
  ...
  Future<void> _parsearGeoJSON(Uint8List bytes) async {
    try {
      ...
      debugPrint(
        '[GEOJSON] parseo completado: total=${parseResult.totalFeatures}, preview=${parseResult.preview.length}',
      );
  ...
  List<Map<String, dynamic>> _geoJsonFeaturesForStorage(
    List<Map<String, dynamic>> features, {
    int maxRows = _maxStoredGeoJsonFeatures,
  }) {
  ...
  List<Map<String, dynamic>> _annotateGeoJsonFeaturesForMap(
    List<Map<String, dynamic>> features,
  ) {
    return features
        .map((feature) {
          final props =
              _asStringDynamicMap(feature['properties']) ?? <String, dynamic>{};
          final clave = props['clave_catastral']?.toString().trim();
          final proyecto =
              GeoJsonMapper.detectarProyecto(props) ??
              GeoJsonMapper.inferProyectoDesdeClave(clave);
          final hasBusinessData =
              (props['propietario_nombre']?.toString().trim().isNotEmpty ??
                  false) ||
              (props['tramo']?.toString().trim().isNotEmpty ?? false) ||
              (props['superficie'] is num && (props['superficie'] as num) > 0);
          final warning =
              (clave == null ||
              clave.isEmpty ||
              proyecto == null ||
              !hasBusinessData);
          final nextProps = Map<String, dynamic>.from(props)
            ..['_gestionWarning'] = warning
            ..['_gestionVinculada'] = !warning;
          return <String, dynamic>{...feature, 'properties': nextProps};
        })
        .toList(growable: false);
  }
  ...
  Future<void> _importarGeoJsonEnModoLocal(
    ...
    final storedFeatures = _geoJsonFeaturesForStorage(features);
    final annotatedForMap = _annotateGeoJsonFeaturesForMap(features);
    final insertadosLocales = ref
        .read(localPrediosProvider.notifier)
        .upsertManyFromGeoJsonFeatures(features, archivoId: fileId);
    ref.invalidate(prediosListProvider);
    ref.invalidate(prediosMapaProvider);

    final proyectoDetectado = GeoJsonMapper.detectarProyectoDesdeFeatures(
      features,
    );
    ...
    try {
      final archivosRepo = ref.read(localArchivosRepositoryProvider);
      final saved = await archivosRepo
          .saveArchivo(
            customId: fileId,
            nombre: nombre,
          ...
          .timeout(
            ...
            onTimeout: () =>
                throw TimeoutException('saveArchivo timeout local'),
          );
    ...
    // Inyectar features completas para render inmediato de mapa en la sesión.
    ref.read(importedFeaturesProvider.notifier).state = annotatedForMap;
    ref.read(mapaColorModeProvider.notifier).state =
        MapaColorMode.estatusPredio;

    ref
        .read(importacionAsyncProvider.notifier)
        .completar(total: features.length, etapa: 'Completado');

    if (!mounted) return;
    final prefijo = (detalle == null || detalle.trim().isEmpty)
        ? 'BD no disponible.'
        : '$detalle\n';
    final notaPreview = features.length > storedFeatures.length
        ? '\nSe guardó vista previa de ${storedFeatures.length} feature(s) para evitar bloqueos.'
        : '';
    _mostrarSnackBar(
      '$prefijo$insertadosLocales predio(s) registrados en Gestion local.$notaPreview',
      exito: true,
    );
    context.go('/tabla');
  }
  ...
  Future<void> _guardarYVerEnMapa() async {
    ...
    if (_geoJsonData == null || _extraerFeatures().isEmpty) {
      try {
        debugPrint(
          '[GEOJSON] estado vacio al guardar; reintentando parseo desde archivo seleccionado',
        );
        final bytes = await _leerBytesArchivoSeleccionado();
        if (bytes == null) {
          _mostrarSnackBar(
            'No se pudo leer el archivo para iniciar la importacion.',
            exito: false,
          );
          return;
        }
        ...
        _mostrarSnackBar(
          'No se pudo preparar el GeoJSON para importar: $e',
          exito: false,
        );
        return;
      }
    ...
    if (_geoJsonData == null) {
      _mostrarSnackBar(
        'No hay datos GeoJSON listos para importar.',
        exito: false,
      );
      return;
    }
    ...
    if (features.isEmpty) {
      _mostrarSnackBar(
        'El archivo no contiene features validos para importar.',
        exito: false,
      );
      return;
    }
    ...
    if (features.length >= _geoJsonLocalFirstThreshold) {
      debugPrint(
        '[GEOJSON] local-first habilitado por volumen: total=${features.length}',
      );
      setState(() => _sincronizando = true);
      ref
          .read(importacionAsyncProvider.notifier)
          .iniciar(total: features.length, etapa: 'Importando local');
      try {
    ...
    ref
        .read(importacionAsyncProvider.notifier)
        .iniciar(total: features.length, etapa: 'Validando en BD');
    debugPrint(
      '[GEOJSON] iniciar guardado/sincronizacion: total=${features.length}, nombre=$nombre',
    );
    ...
    try {
          ...
          .sincronizar(
            ...
            onProgress: (procesados, total) {
              if (!firstProgressLogged) {
                ...
                debugPrint(
                  '[GEOJSON] primer progreso recibido: $procesados/$total',
                );
              }
              ref
                  .read(importacionAsyncProvider.notifier)
      ...
      if (resultado.errores > 0 &&
        ...
        _mostrarSnackBar(
          'No se pudo registrar en Gestión.\n$detalle',
          exito: false,
        );
      ...
      if (totalGestion == 0) {
        ...
        final insertadosLocales = ref
            .read(localPrediosProvider.notifier)
            .upsertManyFromGeoJsonFeatures(features, archivoId: archivoId);
      ...
      ref.read(importedFeaturesProvider.notifier).state =
          _annotateGeoJsonFeaturesForMap(resultado.features);
      ref.read(mapaColorModeProvider.notifier).state =
          MapaColorMode.estatusPredio;
      ...
      debugPrint(
        '[GEOJSON] error en sincronizacion remota, usando fallback local: $e',
      );
  ...
  Future<void> _reprocesarArchivo(ImportedFile file) async {
    ...
    if (fullFeatures.isEmpty) {
      _mostrarSnackBar(
        'El archivo no tiene features para reprocesar.',
        exito: false,
      );
      return;
    }
    ...
    ref
        .read(importacionAsyncProvider.notifier)
        .iniciar(total: fullFeatures.length, etapa: 'Reprocesando');
    ...
    try {
      ...
      ref
          .read(importacionAsyncProvider.notifier)
          .completar(total: file.features.length, etapa: 'Completado');
  ...
  List<Map<String, dynamic>> _rehydrateImportedFeaturesWithIndexes(
    ...
    List<String>? archivoIds,
    Map<String, List<Predio>>? byArchivoId,
  }) {
    ...
    final canUseIndexFallback =
        prediosDelArchivo.isNotEmpty &&
        prediosDelArchivo.length == features.length;
  ...
  Future<void> _eliminarArchivo(ImportedFile file) async {
    ...
    try {
      ...
      try {
        final byMain = await repoPredios
            .getPrediosByArchivoId(file.id)
            .timeout(const Duration(seconds: 3), onTimeout: () => []);
        if (byMain.isNotEmpty) {
          await repoPredios.deletePrediosByArchivoId(file.id);
          eliminadosGestion += byMain.length.toInt();
        }
        ...
        if (eliminadosGestion == 0 &&
          ...
          if (byBdId.isNotEmpty) {
            ...
            eliminadosGestion += byBdId.length.toInt();
          }
        }
      ...
      _actualizarEliminacion(
        procesados: paso,
        etapa: 'Refrescando Gestión y mapa...',
      );
  ...
  Future<void> _eliminarVarios(List<ImportedFile> files) async {
    if (_eliminando) return;
    ...
    try {
      ...
      _actualizarEliminacion(
        procesados: paso,
        etapa: 'Limpiando lista de archivos...',
      );
      ...
      for (final file in files) {
        ...
        try {
          ...
          if (byMain.isNotEmpty) {
            await repoPredios.deletePrediosByArchivoId(file.id);
            borradosArchivo += byMain.length.toInt();
          }
          ...
          if (borradosArchivo == 0 &&
            ...
            if (byBdId.isNotEmpty) {
              await repoPredios.deletePrediosByArchivoId(file.bdId!);
              borradosArchivo += byBdId.length.toInt();
            }
          }
      ...
      _mostrarSnackBar(
        'Eliminación completada. $eliminadosGestion predio(s) removidos de Gestión.',
      );
    } catch (e) {
  ...
  Future<void> _inyectarXlsxLocal(XlsxParseResult parseResult) async {
    ...
    try {
          ...
          try {
            ...
            final existente =
                ref
                    ...
                    .any((item) => item.claveCatastral == clave) ||
      ...
      if (conteo.isNotEmpty) {
        proyectoDetectado = conteo.entries
      ...
      setState(() => _sincronizando = false);
      _mostrarSnackBar(
        'Error al inyectar XLSX en modo local: $e',
        exito: false,
      );
    }
  }
  ...
  Propietario? _findLocalPropietario(
    ...
    final comparador =
        (nombreCompleto ?? [nombre, apellidos].whereType<String>().join(' '))
            .trim()
            .toUpperCase();
  ...
  Future<void> _exportarReporteErrores({required bool asCsv}) async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Reporte de errores generado.')),
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (isBusy ||
                      estadoImportacion != ImportacionEstado.idle) ...[
                    const SizedBox(height: 16),
                    Container(
                      width: double.infinity,
                      padding: const EdgeInsets.all(14),
                      decoration: BoxDecoration(
                        color: _colorEstadoImportacion(
                          estadoImportacion,
                  ...
                  if ((_syncResultado?.errores ?? 0) > 0 ||
                    ...
                    Wrap(
                      ...
                      children: [
                        OutlinedButton.icon(
                          onPressed: () =>
                              _exportarReporteErrores(asCsv: false),
                          icon: const Icon(
                  ...
                  if (_xlsxParseResult != null) ...[
                    ...
                    Container(
                      ...
                      child: Column(
                        children: _xlsxParseResult!.preview.asMap().entries.map(
                          (entry) {
                            final item = entry.value;
                            final tabla =
                                item['tabla']?.toString() ?? 'desconocida';
                            ...
                            return Column(
                              children: [
                                ...
                                if (entry.key <
                                    _xlsxParseResult!.preview.length - 1)
                                  const Divider(height: 1),
                              ],
                            );
                          },
                            ...
                            children: [
                              ...
                              if (_archivosSeleccionados.isNotEmpty)
                                Padding(
                                  ...
                                  child: Text(
                                    '${_archivosSeleccionados.length} seleccionados',
                                    style: const TextStyle(
                                      fontSize: 12,
                                      color: AppColors.textLight,
                                      fontWeight: FontWeight.w600,
                              ...
                              TextButton.icon(
                                onPressed:
                                    ...
                                    ? null
                                    : () {
                                        final seleccionados = importedFiles
                                            .where(
                                              (file) => _archivosSeleccionados
                                                  .contains(file.id),
                                            )
                                            .toList(growable: false);
                                        showDialog(
                                          ...
                                          builder: (ctx) => AlertDialog(
                                            ...
                                            content: Text(
                                              '¿Eliminar ${seleccionados.length} archivo(s) seleccionados? Se borrarán también de la base de datos.',
                                            ),
                                            actions: [
                                              TextButton(
                                                onPressed: () =>
  ...
  Widget _buildArchivoTile(ImportedFile file) {
    final statusColor = file.guardadoEnBD
        ? AppColors.secondary
        : AppColors.textLight;
    final statusIcon = file.guardadoEnBD
        ? Icons.cloud_done_outlined
        : Icons.cloud_off_outlined;
    final statusLabel = file.guardadoEnBD
        ? 'Guardado en BD'
        : 'Solo en memoria';

    final busy = _loading || _eliminando;
    final selected = _archivosSeleccionados.contains(file.id);
          ...
          children: [
            Checkbox(
              ...
              onChanged: busy
                  ? null
                  : (value) =>
                        _toggleArchivoSeleccionado(file.id, value ?? false),
              visualDensity: VisualDensity.compact,
