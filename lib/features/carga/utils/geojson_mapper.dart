...
class GeoJsonMapper {
  ...
  static const _keyAliases = <String, List<String>>{
    ...
    'zona': ['zona', 'ZONA', 'sector', 'SECTOR', 'region', 'REGION'],
    'valor_catastral': [
      'valor_catastral',
      'VALOR_CATASTRAL',
      'valor',
    ...
    for (final entry in _keyAliases.entries) {
      ...
      if (isNullLike(selected)) {
        ...
        if (selected == null && canonicalKey == 'tramo') {
          for (final prop in props.entries) {
            ...
            final looksLikeTramo =
                keyNorm.contains('tramo') ||
      ...
      if (!isNullLike(selected)) {
        if (canonicalKey == 'tramo') {
          ...
          result[canonicalKey] = _normalizeCanonicalValue(
            canonicalKey,
            selected,
          );
        }
  ...
  static String _normalizeTramoTsfValue(
    dynamic value, {
    ...
    final prefijoPorTexto = raw.contains('SEGMENTO')
        ...
        : (raw.contains('FRENTE')
              ? 'F'
              : (raw.contains('TRAMO') ? 'T' : defaultPrefix));
  ...
  static String _normalizeEstatus(String value) {
    return normalizeEstatusNullable(value) ?? 'Sin estatus';
  }

  static String? normalizeEstatusNullable(dynamic rawValue) {
    if (rawValue == null) return null;
    final raw = _normalizeSpaces(rawValue.toString());
    ...
    final isSinEstatus =
        text.contains('sin estatus') ||
        compact.contains('sinestatus') ||
        text.contains('sin estado') ||
        compact.contains('sinestado') ||
        text.contains('sin dato') ||
        compact.contains('sindato');
    if (isSinEstatus) return 'Sin estatus';
  ...
  static String? detectarProyecto(Map<String, dynamic> props) {
    ...
    for (final entry in props.entries) {
      ...
      final keyIsProjectLike =
          key.contains('proyecto') ||
          key.contains('obra') ||
