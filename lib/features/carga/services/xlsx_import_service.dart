...
enum XlsxTargetTable { predios, propietarios }
...
class XlsxImportService {
  ...
  static const Map<String, List<String>> _prediosAliases = {
    ...
    'tramo': ['tramo', 'zona', 'segmento', 'sector', 'modulo', 'tramo_vial'],
    'tipo_propiedad': [
      'tipo_propiedad',
      'tipo',
      'uso_suelo',
    ...
    'rfc_propietario': ['rfc_propietario', 'rfc', 'rfc_titular'],
    'curp_propietario': ['curp_propietario', 'curp', 'curp_titular'],
    ...
    'negociacion': ['negociacion', 'negociado', 'negociacion_si', 'neg'],
    'poligono_insertado': [
      'poligono_insertado',
      'poligono',
      'polig',
  ...
  XlsxParseResult parse(Uint8List bytes) {
    ...
    for (final hoja in workbook.tables.keys) {
      ...
      for (final row in rows.take(3)) {
        preview.add({
          'hoja': hoja,
          'tabla': target == XlsxTargetTable.predios
              ? 'predios'
              : 'propietarios',
          ...row,
        });
  ...
  _HeaderDetection? _detectarHeaderYTabla(List<List<dynamic>> allRows) {
    ...
    for (var rowIndex = 0; rowIndex < maxRowsToScan; rowIndex++) {
      final rawHeaders = allRows[rowIndex]
          .map(_cellToText)
          .toList(growable: false);
      ...
      final totalScore = prediosScore > propietariosScore
          ? prediosScore
          : propietariosScore;
  ...
  Map<String, dynamic> _normalizarFilaPredio(
    ...
    final propietarioNombre = pick(
      _prediosAliasesNormalized['propietario_nombre']!,
    );
    if (propietarioNombre != null)
      out['propietario_nombre'] = _normalizePlainText(propietarioNombre);
    ...
    final identificacion = pickBool(
      _prediosAliasesNormalized['identificacion']!,
    );
    ...
    final poligonoInsertado = pickBool(
      _prediosAliasesNormalized['poligono_insertado']!,
    );
    if (poligonoInsertado != null)
      out['poligono_insertado'] = poligonoInsertado;
    ...
    if (curpProp != null)
      out['curp_propietario'] = _normalizeUpperCode(curpProp);

    final telProp = pick(_prediosAliasesNormalized['telefono_propietario']!);
    if (telProp != null)
      out['telefono_propietario'] = _normalizePlainText(telProp);

    final correoProp = pick(_prediosAliasesNormalized['correo_propietario']!);
    if (correoProp != null)
      out['correo_propietario'] = _normalizeEmail(correoProp);
  ...
  static Map<String, List<String>> _normalizeAliases(
    ...
    return aliases.map(
      (key, values) =>
          MapEntry(key, values.map(_normalize).toList(growable: false)),
    );
  }
